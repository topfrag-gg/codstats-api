...
function Ue(t) {
	...
	return (
		e
		|| (t?.config.engineType === 'library' ? 'library'
		: t?.config.engineType === 'binary' ? 'binary'
		: t?.config.engineType === 'client' ? 'client'
		: Bs(t))
	);
}
/**
...
function xi(t) {
	...
	return (
		t instanceof xe ? new G(`Prisma.${t._getName()}`)
		: Ke(t) ? new G(`prisma.${ve(t.modelName)}.$fields.${t.name}`)
		: Array.isArray(t) ? Ca(t)
		: typeof t === 'object' ? wi(t)
...
function po(t, e) {
	return (
		t ?
			...
			: t.datasourceUrl ? { [e[0]]: { url: t.datasourceUrl } }
			: {}
		:	{}
	);
}
...
function Cl(t) {
	return (
		t instanceof ArrayBuffer || t instanceof SharedArrayBuffer ? !0
	...
	class e {
		...
		constructor(n) {
			...
			try {
				...
				let k = h.engine || {};
				if (
					(f.errorFormat ? (this._errorFormat = f.errorFormat)
					: g.env.NODE_ENV === 'production' ? (this._errorFormat = 'minimal')
					: g.env.NO_COLOR ? (this._errorFormat = 'colorless')
